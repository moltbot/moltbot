...
function tryFlattenNullableAnyOf(
  ...
  for (const variant of variants) {
    ...
    if (v.type === "null") {
      ...
      // This is the non-null variant
      if (nonNullVariant) return null; // More than one non-null variant
      nonNullVariant = v;
    } else {
      return null; // Complex variant, can't flatten
    }
  }

  if (hasNull && nonNullVariant) {
    // Return a copy of the non-null variant (null is just dropped for Gemini)
